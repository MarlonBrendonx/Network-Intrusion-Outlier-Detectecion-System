...
def generate_umap_3d(
    ...
    ax.scatter(
        ...
        p_train_norm[:, 2],
        c="lightgrey",
        label=f"{train_stem}: Normal (Baseline)",
        alpha=0.2,
        s=10,
    )
    ...
    if len(p_target_att) > 0:
        ax.scatter(
            ...
            c="red",
            marker="^",
            label=f"{target_stem}: Ataque",
            alpha=0.8,
            s=40,
        )

    ax.set_title(
        f"Análise de Generalização e Drift: UMAP 3D ({config.algorithm.value})",
        fontsize=16,
    )
